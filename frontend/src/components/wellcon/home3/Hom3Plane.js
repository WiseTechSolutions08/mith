// import element6 from '../../../assets/images/elements/elements6.png'
// import element7 from '../../../assets/images/elements/element7.png'

const Hom3Plane = () => {
  return (
    <div className="bg-thm-color-two plane_box relative z-1">
      <div className="container relative z-1">
        <div className="row justify-content-center">
          <div className="col-xl-9 col-lg-11 relative z-1">
            {/* <img
              src={element6}
              className="element_1 rotate_elem"
              alt="Element"
            /> */}
            <h2 className="thm-color-white">
              The Life Coach School Has The Most Amazing Tools And Cutting-Edge
              Training
            </h2>
          </div>
        </div>
        {/* <img
          src={element7}
          className="element_2 zoom-fade icon-default-height"
          alt="Element"
        /> */}
      </div>
    </div>
  );
};

export default Hom3Plane;
